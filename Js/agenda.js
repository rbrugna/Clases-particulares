document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function cargarTurnos() {
        ...
        turnos = data.        // filepath: /Users/rochu/Library/CloudStorage/OneDrive-BluePrisma/ITBA/2do año/1er cuatri/Programacion Web/Clases particulares/Js/agenda.js
        document.addEventListener("DOMContentLoaded", async function () {
            // role protection: only allow profesora here
            const role = localStorage.getItem('role');
            if (role !== 'profesora') {
            ...
            function mostrarTurnos() {
                ...
                // Table shows turnos; if none, still show message row
                if (turnos.length === 0) {
                    tabla.querySelector('tbody').innerHTML = "<tr><td colspan='6'>No hay horarios cargados.</td></tr>";
                    return;
                }
                ...
                for (let i = 0; i < turnos.length; i++) {
                    ...
                    if (t.editando) {
                        tabla.querySelector('tbody').innerHTML += `
                            <tr>
                            <td>
                                <select id="dia-${i}" class="form-select form-select-sm">
                            ...
                            <td><input type="time" id="hora-${i}" class="form-control form-control-sm" value="${t.hora}"></td>
                            <td><input type="text" id="materia-${i}" class="form-control form-control-sm" value="${t.materia || ""}"></td>
                            <td><input type="text" id="alumno-${i}" class="form-control form-control-sm" value="${t.alumno || ""}"></td>
                            <td>
                                <button class="guardar btn btn-sm btn-primary" data-index="${i}">Guardar</button>
                                <button class="cancelar btn btn-sm btn-secondary" data-index="${i}">Cancelar</button>
                            </td>
                            </tr>`;
                    } else {
                        ...
                        tabla.querySelector('tbody').innerHTML += `
                            ...
                            <td>${t.alumno || "-"}</td>
                            <td>
                                <button class="editar btn btn-sm btn-outline-primary" data-index="${i}">Editar</button>
                                <button class="eliminar btn btn-sm btn-outline-danger" data-index="${i}">Eliminar</button>
                            </td>
                            </tr>`;
                    }
                }
